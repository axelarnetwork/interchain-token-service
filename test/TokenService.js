...
const SELECTOR_RECEIVE_TOKEN_WITH_DATA = 2;
...
describe('Interchain Token Service', () => {
    ...
    deployFunctions.lockUnlock = async function deployNewLockUnlock(
        ...
        const tokenId = await service.customTokenId(wallet.address, salt);
    ...
    describe('Register Canonical Token', () => {
        ...
        beforeEach(async () => {
            ...
            await (await token.setTokenManager(await service.tokenManagerAddress(tokenId))).wait();
        ...
        it('Should revert when registering a canonical token if paused', async () => {
            ...
            await tx.wait();
            await expectRevert((gasOptions) => service.registerCanonicalToken(token.address, gasOptions), service, 'Pause');
    ...
    describe('Initiate Remote Canonical Token Deployment', () => {
        ...
        beforeEach(async () => {
            ...
            tokenId = await service.canonicalTokenId(token.address);
            await token.setTokenManager(await service.tokenManagerAddress(tokenId)).then((tx) => tx.wait());
        ...
        it('Should revert if token manager for given tokenID is not a canonical token manager', async () => {
            ...
            const gasValue = 1e6;
            const expectedCanonicalTokenId = await service.canonicalTokenId(tokenAddress);
            await expectRevert(
    ...
    describe('Custom Token Manager Deployment', () => {
        ...
        it('Should revert when deploying a custom token manager if paused', async () => {
            ...
            await expectRevert((gasOptions) => service.deployCustomTokenManager(salt, LOCK_UNLOCK, params, gasOptions), service, 'Pause');
        ...
        for (const type of ['lockUnlock', 'mintBurn', 'lockUnlockFee']) {
            it(`Should be able to initiate an interchain token transfer [${type}]`, async () => {
                ...
                const payload = defaultAbiCoder.encode(
                    ...
                    [SELECTOR_RECEIVE_TOKEN, tokenId, hexlify(wallet.address), destAddress, sendAmount],
                );
    ...
    describe('Send Token With Data', () => {
        ...
        for (const type of ['lockUnlock', 'mintBurn', 'lockUnlockFee']) {
            it(`Should be able to initiate an interchain token transfer [${type}]`, async () => {
                ...
                const payload = defaultAbiCoder.encode(
                    ...
                    [SELECTOR_RECEIVE_TOKEN_WITH_DATA, tokenId, sourceAddress, destAddress, sendAmount, data],
                );
                const payloadHash = keccak256(payload);
        ...
        for (const type of ['lockUnlock', 'mintBurn', 'lockUnlockFee']) {
            it(`Should be able to initiate an interchain token transfer via the interchainTransfer function on the service [${type}]`, async () => {
                ...
                const payload = defaultAbiCoder.encode(
                    ['uint256', 'bytes32', 'bytes', 'bytes', 'uint256', 'bytes'],
                    [SELECTOR_RECEIVE_TOKEN_WITH_DATA, tokenId, sourceAddress, destAddress, sendAmount, '0x'],
                );
                const payloadHash = keccak256(payload);
    ...
    describe('Express Execute', () => {
        ...
        it('Should express execute', async () => {
            const payload = defaultAbiCoder.encode(
                ['uint256', 'bytes32', 'bytes', 'bytes', 'uint256'],
                [SELECTOR_RECEIVE_TOKEN, tokenId, hexlify(wallet.address), destinationAddress, amount],
            );
        ...
        it('Should express execute with token', async () => {
            const payload = defaultAbiCoder.encode(
                ['uint256', 'bytes32', 'bytes', 'bytes', 'uint256', ' bytes'],
                [SELECTOR_RECEIVE_TOKEN_WITH_DATA, tokenId, sourceAddress, executable.address, amount, data],
            );
            await expect(service.expressExecute(commandId, sourceChain, sourceAddress, payload))
                .to.emit(service, 'ExpressExecuted')
                .withArgs(commandId, sourceChain, sourceAddress, keccak256(payload), wallet.address)
    ...
    describe('Flow Limits', () => {
        ...
        it('Should be able to receive token only if it does not trigger the mint limit', async () => {
            ...
            async function receiveToken(sendAmount) {
                const payload = defaultAbiCoder.encode(
                    ['uint256', 'bytes32', 'bytes', 'bytes', 'uint256'],
                    [SELECTOR_RECEIVE_TOKEN, tokenId, hexlify(wallet.address), wallet.address, sendAmount],
                );
