 ...
 */
contract InterchainTokenServiceVirtual is
    Upgradable,
    Operator,
    Pausable,
    Multicall,
    ...
    IInterchainTokenService
{
    ...
    function interchainTransfer(
        ...
        (ICallContract.MetadataVersion metadataVersion, bytes memory data) = _decodeMetadata(metadata);
    ...
    function callContractWithInterchainToken(
        ...
        _transmitInterchainTransfer(
            ...
            destinationAddress,
            amount,
            ICallContract.MetadataVersion.CONTRACT_CALL,
            data,
            symbol,
            gasValue
        );
    ...
    function _callContract(
        ...
        if (gasValue > 0) {
            if (metadataVersion == ICallContract.MetadataVersion.CONTRACT_CALL) {
            ...
            } else if (metadataVersion == ICallContract.MetadataVersion.EXPRESS_CALL) {
    ...
    function _deployRemoteTokenManager(
        ...
        _callContract(destinationChain, payload, ICallContract.MetadataVersion.CONTRACT_CALL, gasValue);
    }
     ...
     */
    function _decodeMetadata(bytes calldata metadata) internal pure returns (ICallContract.MetadataVersion version, bytes memory data) {
        if (metadata.length < 4) return (ICallContract.MetadataVersion.CONTRACT_CALL, data);
        ...
        version = ICallContract.MetadataVersion(versionUint);
